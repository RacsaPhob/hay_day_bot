...
def go_through_field():
    for i in range(8):
        pyautogui.moveRel(700, -350, 0.13)
        pyautogui.moveRel(10, 10)
        pyautogui.moveRel(-700, 350, duration=0.13)
        pyautogui.moveRel(10, 10)
...
if __name__ == '__main__':
    quantity = int(input('введите текущее количество пшеницы в силосе'))
    storage = int(input('введите вместительность силоса'))
    harvest_value = int(input('введите количество полей'))
    sleep(3)

    bot = Bot(quantity, storage, harvest_value)
    while True:
        bot.run()
        sleep(110)
